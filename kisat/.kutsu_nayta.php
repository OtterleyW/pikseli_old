...
?>
	...
	<p>
		<ul>
			...
			<li>Tulokset arvotaan suhteutetulla arvonnalla</li>
		</ul>
	...
	<ul>
		...
		<ul>
			<li>Osallistuminen sähköpostiin <b><a href="mailto:<?= $osallistuminen_mail ?>"><?= $osallistuminen_mail ?></a></b> otsikolla <b><?= "$osallistuminen_otsikko $pienin_kutsu_id-$suurin_kutsu_id" ?></b><br />
			<small>Samalla osallistumisella osallistut ryppään jokaiseen kutsuun. Et voi osallistua yksittäiseen kutsuun erikseen.</small></li>
	...
	<?
		$kutsujen_linkit = array();

		foreach(range($pienin_kutsu_id, $suurin_kutsu_id) as $kutsun_id){
			$kutsun_linkki = "<a href=\"?id=$kutsun_id\">$kutsun_id</a>";
			$kutsujen_linkit[] = $kutsun_linkki;
		} 
	?>
		<p>
		Ryppääseen kuuluvat kutsut: 
		<?
		 echo implode(", ", $kutsujen_linkit);
		?>
		</p>
	...
	if($tulokset_on_arvottu){ ?>
		...
		<h2>Tulokset</h2>
			<? foreach ($luokat as $numero => $luokka) { ?>
				...
				<h3><?= $numero + 1 ?>. <?= $luokka['nimi'] ?></h3>

				<p>
					<?
					$tulokset = pilko_rivit($luokka['tulokset']);
					tulosta_tulokset($tulokset);
					  ?>
				</p>
