...
?>
	...
	<ul>
		...
		<ul>
			<li>Osallistuminen sähköpostiin <b><a href="mailto:<?= $osallistuminen_mail ?>"><?= $osallistuminen_mail ?></a></b> otsikolla <b><?=$osallistuminen_otsikko ?></b><br /></li>
	...
	if($tulokset_on_arvottu){ ?>
		...
		<h2>Tulokset</h2>
			...
			?>
				...
				<h3><?= $numero + 1 ?>. <?= $luokka['nimi'] ?> (<?=count($tulokset) ?> os.)</h3>
...
?>
	<h2>Luokat</h2>
		...
		<ol>
			...
			?>
			<li><?= $luokka['nimi'] ?> (<?=$osallistuja_lkm ?> os.)</li>
	...
	<h2>Osallistujat</h2>
		...
		<? foreach ($luokat as $numero => $luokka) { 
			...
			$osallistuja_lkm = count($osallistuja_array);
		?>
				<h3><?= $numero + 1 ?>. <?= $luokka['nimi'] ?> <?=$osallistuja_lkm ?> os.</h3>
